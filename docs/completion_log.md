...
| 2026-06-11 16:30 | 2026-06-11 16:34 | no-cold-start policy update | pass | Removed cold-start/repeated load-unload gate language. Added `unload_after_run` / `UNLOAD_AFTER_RUN` with default `false`, process-local llama.cpp and MLC model reuse, and AIME26 matrix behavior that unloads only after each backend's final batch. Device start temperature is not a gate; battery/thermal readings remain report-only diagnostics. Build passed, but this APK was not installed so the currently running device task would not be interrupted. |
